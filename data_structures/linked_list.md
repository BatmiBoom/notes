...
A linked list is a collection of ordered items, so its similar to a "normal" list (also called an "array" or "slice" in other languages)
...
Linked lists can be annoying to use and incur more overhead, so why use a linked list at all? It's because sometimes linked lists are much faster to make updates to, particularly when inserting or deleting items from the middle.
