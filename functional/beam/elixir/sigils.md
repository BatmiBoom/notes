...
We have double-quoted strings as well as a concept called charlists, which are defined using the `~c"hello world"` sigil syntax.
...
Elixir provides Perl-compatible regular expressions, as implemented by the PCRE library. Regexes also support modifiers. For example, the `i` modifier makes a regular expression case insensitive:
...
The reason behind supporting different delimiters is to provide a way to write literals without escaped delimiters. For example, a regular expression with forward slashes like `~r(^https?://)` reads arguably better than `~r/^https?:\/\//`.
...
The `~w` sigil also accepts the `c` (charlist), `s` (string) and `a` (atom) modifiers, which specify the data type of the elements of the resulting list:
...
- `~s` will return string and allows escape codes and interpolation
- `~S` will return string but doesn't allow escape codes or interpolation
...
Sigils are extensible. In fact, using the sigil `~r/foo/i` is equivalent to calling `sigil_r` with a binary and a charlist as the argument:
...
Let's implement the ~i sigil that returns an integer (with the optional n modifier to make it negative):
